...
inline void WindCalculations_float ( in float3 i_vertexPos, in float2 i_widthHeight, in float3 i_flex, in float3 i_frequency, out float3 o_vertexPos )
{
	...
	#ifdef _PW_FLORAHDRP_ON
		float4x4 windObjectToWorld = UNITY_MATRIX_M;
		float4x4 windWorldToObject = UNITY_MATRIX_I_M;
	#elif defined(_PW_FLORAHDRP)
	    float4x4 windObjectToWorld = UNITY_MATRIX_M;
		float4x4 windWorldToObject = UNITY_MATRIX_I_M;
	#else
		float4x4 windObjectToWorld = unity_ObjectToWorld;
		float4x4 windWorldToObject = unity_WorldToObject;
	#endif

	#ifdef _PW_FLORAHDRP_ON
		float3 worldOffset = windObjectToWorld._m03_m13_m23 + _WorldSpaceCameraPos; // grab pos from matrix translation
	#elif defined(_PW_FLORAHDRP)
		float3 worldOffset = windObjectToWorld._m03_m13_m23 + _WorldSpaceCameraPos; // grab pos from matrix translation
	#else
		float3 worldOffset = windObjectToWorld._m03_m13_m23; // grab pos from matrix translation
	#endif
	...
	if(nrmSqrdDist > 0.0f)
	{
		...
		float3 zScaleRotVec = mul((float3x3)windObjectToWorld,up);
		...
		float3 localWorldPos = mul((float3x3)windObjectToWorld,i_vertexPos.xyz);
		...
		o_vertexPos = mul((float3x3)windWorldToObject,localWorldPos.xyz);
	}
