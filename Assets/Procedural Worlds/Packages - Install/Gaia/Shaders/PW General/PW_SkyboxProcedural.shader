...
Shader "PWS/Skybox/PW_Procedural" {
Properties {
    ...
    _FogHeight ("Fog Height", Range(-1000,8000)) = 0
    _FogGradient ("Fog Gradient", Range(0,1)) = 0.1
}
...
//              for(int i=0; i<int(kSamples); i++) // Loop removed because we kept hitting SM2.0 temp variable limits. Doesn't affect the image too much.
            ...
            // todo write our own fog functions to avoid overly complex clamp.
            float3 worldPos = mul((float3x3)unity_ObjectToWorld, v.vertex.xyz);
            float3 clampedPos = max(worldPos,-_ProjectionParams.zzz + 0.1);
            clampedPos = min(clampedPos,_ProjectionParams.zzz - 0.1 );
            
            float3 clampedFogPos = mul((float3x3)unity_WorldToObject, clampedPos);
            UNITY_TRANSFER_FOG(OUT,UnityObjectToClipPos(float4(clampedFogPos.xyz,v.vertex.w)));
        ...
        // if y < 0 [eyeRay.y > 0] - sky
            ...
            half3 fog = col;
            UNITY_APPLY_FOG(IN.fogCoord, fog);

            col = lerp(fog,col,1-pow(1-saturate((-y -_FogHeight)*(1-_FogGradient)),2));
