...
namespace Gaia
{
    ...
    public class GaiaTerrainExplorerEditor : EditorWindow
    {
        ...
        private void ExportMask()
        {
            ...
            if (Terrain.activeTerrain != null)
            {
                path = Path.Combine(path, PWCommon5.Utils.FixFileName(Terrain.activeTerrain.name + "_Mask"));
            }
            else
            {
                path = Path.Combine(path, PWCommon5.Utils.FixFileName("Mask"));
            }
        ...
        private void SetupMask()
        {
            ...
            else
            {
                ...
                if (m_useErosionMask1)
                {
                    ...
                    {
                        if (!PWCommon5.Utils.Math_ApproximatelyEqual(m_erosionMaskStrength1, 1f))
                        {
                            workingMask1.MultiplyClamped(m_erosionMaskStrength1, 0f, 1f);
                        }
                        if (!PWCommon5.Utils.Math_ApproximatelyEqual(m_erosionMaskOffset1, 0f))
                        {
                            workingMask1.AddClamped(m_erosionMaskOffset1, 0f, 1f);
                        }
                        if (!PWCommon5.Utils.Math_ApproximatelyEqual(m_erosionMaskPowerOf1, 1f))
                        {
                            workingMask1.Power(m_erosionMaskPowerOf1);
                        }
                        if (!PWCommon5.Utils.Math_ApproximatelyEqual(m_erosionMaskContrast1, 1f))
                        {
                            workingMask1.Contrast(m_erosionMaskContrast1);
                        }
                        if (!PWCommon5.Utils.Math_ApproximatelyEqual(m_quantizeMask1, 0f))
                        {
            ...
            else
            {
                ...
                if (m_useErosionMask2)
                {
                    ...
                    {
                        if (!PWCommon5.Utils.Math_ApproximatelyEqual(m_erosionMaskStrength2, 1f))
                        {
                    ...
                    for (int tileZ = 0; tileZ < tilesZ; tileZ++)
                    {
                        switch (m_operationType)
                        {
                            ...
                            case GaiaConstants.TerrainOperationType.HydraulicFilter:
                                ...
                                string fname = path;
                                fname = Path.Combine(path, PWCommon5.Utils.FixFileName(mgr.PhysicalTerrainArray[tileX, tileZ].name + "_Sed"));
            ...
            if (mgr.TileCount > 0)
            {
                ...
                for (int tileX = 0; tileX < tilesX; tileX++)
                {
                    for (int tileZ = 0; tileZ < tilesZ; tileZ++)
                    {
                        ...
                        string fname = path;
                        fname = Path.Combine(path, PWCommon5.Utils.FixFileName(mgr.PhysicalTerrainArray[tileX, tileZ].name + "_HeightMap"));
        ...
        private void ExportFlowmap()
        {
            ...
            if (mgr.TileCount > 0)
            {
                ...
                for (int tileX = 0; tileX < tilesX; tileX++)
                {
                    ...
                    {
                        string fname = path;
                        fname = Path.Combine(path, PWCommon5.Utils.FixFileName(mgr.PhysicalTerrainArray[tileX, tileZ].name + "_FlowMap"));
        ...
        private void ExportCuvaturemap()
        {
            ...
            if (mgr.TileCount > 0)
            {
                ...
                for (int tileX = 0; tileX < tilesX; tileX++)
                {
                    ...
                    {
                        string fname = path;
                        fname = Path.Combine(path, PWCommon5.Utils.FixFileName(mgr.PhysicalTerrainArray[tileX, tileZ].name + "_CurvatureMap"));
        ...
        private void ExportSlopemap()
        {
            ...
            if (mgr.TileCount > 0)
            {
                ...
                for (int tileX = 0; tileX < tilesX; tileX++)
                {
                    ...
                    {
                        string fname = path;
                        fname = Path.Combine(path, PWCommon5.Utils.FixFileName(mgr.PhysicalTerrainArray[tileX, tileZ].name + "_SlopeMap"));
        ...
        private void ExportAspectMap()
        {
            ...
            if (mgr.TileCount > 0)
            {
                ...
                for (int tileX = 0; tileX < tilesX; tileX++)
                {
                    ...
                    {
                        string fname = path;
                        fname = Path.Combine(path, PWCommon5.Utils.FixFileName(mgr.PhysicalTerrainArray[tileX, tileZ].name + "_AspectMap"));
        ...
        private void ExportNormalmap()
        {
            ...
            if (mgr.TileCount > 0)
            {
                ...
                for (int tileX = 0; tileX < tilesX; tileX++)
                {
                    ...
                    {
                        string fname = path;
                        fname = Path.Combine(path, PWCommon5.Utils.FixFileName(mgr.PhysicalTerrainArray[tileX, tileZ].name + "_NormalMap"));
                        ...
                        PWCommon5.Utils.WriteAllBytes(fname+".exr", exrBytes);
                        MonoBehaviour.DestroyImmediate(nrmTexture);

                        AssetDatabase.Refresh();
                        Texture2D normalTex = PWCommon5.AssetUtils.GetAsset(fname, typeof(Texture2D)) as Texture2D;
        ...
        private void ExportNoisemap()
        {
            ...
            if (mgr.TileCount > 0)
            {
                ...
                for (int tileX = 0; tileX < tilesX; tileX++)
                {
                    ...
                    {
                        string fname = path;
                        fname = Path.Combine(path, PWCommon5.Utils.FixFileName(mgr.PhysicalTerrainArray[tileX, tileZ].name + "_NoiseMap"));
            ...
            for (int x = 0; x < width; x++)
            {
                for (int y = 0; y < height; y++)
                {
                    ...
                    if (maskValue > splatValue)
                    {
                        ...
                        float adjustment = 0f;
                        if (!PWCommon5.Utils.Math_ApproximatelyEqual(theRest, 0f))
                        {
