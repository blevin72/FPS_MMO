        ...
        private void GlobalSettings(bool helpEnabled)
        {
            ...
            if (m_profile.m_useCaustics)
            {
                ...
                if (m_profile.m_editCausticTextures)
                {
                    ...
                    {
                        for (int i = 0; i < m_profile.m_causticTexturesHDRP.Count; i++)
                        {
                            EditorGUILayout.BeginHorizontal();
                            m_profile.m_causticTexturesHDRP[i] = (Texture2D)EditorGUILayout.ObjectField(new GUIContent("[" + i + "]" + m_editorUtils.GetTextValue("CausticTexture"), m_editorUtils.GetTooltip("CausticTexture")), m_profile.m_causticTexturesHDRP[i], typeof(Texture2D), false, GUILayout.MaxHeight(16f));
                            if (m_editorUtils.Button("Remove", GUILayout.MaxWidth(30f)))
                            {
                                m_profile.m_causticTexturesHDRP.RemoveAt(i);
                            }
                            EditorGUILayout.EndHorizontal();
                        }
                        if (m_editorUtils.Button("Add"))
                        {
                            m_profile.m_causticTexturesHDRP.Add(null);
                        }
                    }
                    else
                    {
